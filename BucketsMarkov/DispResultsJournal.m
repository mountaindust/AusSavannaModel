...
location_array = {'Katherine','Darwin','Sydney'};
eqTBAvals = [10.5, 13.1, 24.7];
label_size = 16;
tick_size = 14;
legend_size = 12.6;
title_size = 24;
...
for loc = 1:length(location_array)
    ...
    hold on
    if loc == 1
        l1 = plot(total_tba,nostressprob,'LineStyle','--','Color',[0.55 0.27 0.07]);
        sl1 = plot(total_tba,juvsurvprob,'LineStyle','--','Color',[0.82 0.71 0.63]);
        sc1 = scatter(eqTBAvals(loc),...
            nostressprob(round(eqTBAvals(loc)/(total_tba(2)-total_tba(1)))),...
            50,'k','*');
    elseif loc == 2
        l2 = plot(total_tba,nostressprob,'Color',[1 0.55 0]);
        sl2 = plot(total_tba,juvsurvprob,'Color',[1 0.82 0.6]);
        scatter(eqTBAvals(loc),...
            nostressprob(round(eqTBAvals(loc)/(total_tba(2)-total_tba(1)))),...
            50,'k','*');
    else
        l3 = plot(total_tba,nostressprob,'-.','Color',[0.09 0.38 0.09]);
        sl3 = plot(total_tba,juvsurvprob,'LineStyle','-.','Color',[0.64 0.75 0.64]);
        scatter(eqTBAvals(loc),...
            nostressprob(round(eqTBAvals(loc)/(total_tba(2)-total_tba(1)))),...
            50,'k','*')
    ...
    if loc == 1
        mdry = mean(drydays);
        d1 = plot(total_tba,mdry,'LineStyle','--','Color',[0.55 0.27 0.07]);
        scd1 = scatter(eqTBAvals(loc),...
            mdry(round(eqTBAvals(loc)/(total_tba(2)-total_tba(1)))),...
            50,'k','*');
    elseif loc == 2
        mdry = mean(drydays);
        d2 = plot(total_tba,mdry,'Color',[1 0.55 0]);
        scatter(eqTBAvals(loc),...
            mdry(round(eqTBAvals(loc)/(total_tba(2)-total_tba(1)))),...
            50,'k','*');
    else
        mdry = mean(drydays);
        d3 = plot(total_tba,mean(drydays),'LineStyle','-.','Color',[0.09 0.38 0.09]);
        scatter(eqTBAvals(loc),...
            mdry(round(eqTBAvals(loc)/(total_tba(2)-total_tba(1)))),...
            50,'k','*');
    ...
    elseif strcmp(tbain,'q') || strcmp(tbain,'Q')
        ...
        elseif mod(tba,total_tba(2)-total_tba(1)) ~= 0
            ...
            histogram(dryend(:,param)-drystart(:,param),...
                ...
                'Normalization','probability','FaceColor',[1 0.55 0])
            set(gca,'FontSize',tick_size)
            ...
            set(h,'FaceColor','k','EdgeColor',[1 0.55 0])
            ...
            histogram(drystart(:,param),1:365,'Normalization','probability',...
                'FaceColor',[.6 .4 0],'EdgeColor',[.6 .4 0])
            hold on
            %end of dry
            histogram(dryend(:,param),1:max(dryend(:,param)),...
                'Normalization','probability',...
                'FaceColor',[0.39 0.58 0.93],'EdgeColor',[0.39 0.58 0.93])
            set(gca,'FontSize',tick_size)
